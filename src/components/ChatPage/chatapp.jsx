...
export default function ModernChat() {
    ...
    try {
      ...
      socket.on("call-accepted", async ({ from, roomId }) => {
        ...
        try {
          ...
          console.log("Sent WebRTC offer to:", from);
        } catch (error) {
  ...
  useEffect(() => {
    if (selectedUser && !isInRandomChat) {
      fetchMessagesForUser(selectedUser);
    }
  }, [selectedUser, isInRandomChat, fetchMessagesForUser]);
  ...
  const startCall = async (type) => {
    ...
    try {
      ...
      setCallType(type);
      setInCall(true);
      setCallParticipants([receiver]);
      setCallStatus("Calling...");

      if (socketRef.current) {
        console.log("Emitting initiate-call to:", receiver._id);
  ...
  // FIXED: Improved acceptCall function
  const acceptCall = async () => {
    ...
    try {
      ...
      const stream = await navigator.mediaDevices.getUserMedia(constraints);
      console.log("Got media stream for incoming call:", stream.getTracks());
      ...
      if (socketRef.current) {
        console.log("Emitting accept-call to:", incomingCall.caller._id);
        ...
        <div className="fixed inset-0 bg-slate-950 z-40 flex flex-col">
          ...
          <div className="flex items-center justify-between px-8 py-6 bg-slate-900/95 backdrop-blur-md border-b border-slate-800 shadow-lg">
            <div className="flex items-center space-x-6">
              ...
              <div>
                <h3 className="font-bold text-white text-xl">{selectedUser?.username || randomMatch?.username}</h3>
                    ...
                    <div key={participant._id} className="relative bg-gradient-to-br from-slate-800 to-slate-900 rounded-3xl overflow-hidden border-2 border-slate-700 hover:border-cyan-500/50 transition-all shadow-2xl group">
                      {remoteVideosRef.current[participant._id] ? (
                        <video
                          ref={(el) => {
                            if (!el) return;
                            const stream = remoteVideosRef.current[participant._id];
                            if (stream && el.srcObject !== stream) {
                              el.srcObject = stream;
                              el.play().catch(err => console.warn('Could not play remote video:', err));
                            }
                          }}
                          autoPlay
                          playsInline
                        ...
                        <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-slate-800 to-slate-900">
                          <div className="text-center">
                            ...
                            <p className="text-slate-400 text-sm mt-2">Connecting video...</p>
                          </div>
                        </div>
                      )}
                      <div className="absolute bottom-4 left-4 bg-black/80 px-4 py-2 rounded-full text-sm backdrop-blur-md border border-slate-700/50 shadow-lg">
            ...
            <div className="flex items-center space-x-6 flex-1 min-w-0">
              <div className="relative">
                <div className={`w-14 h-14 rounded-full flex items-center justify-center text-white font-semibold shadow-lg ${
                  ...
                  {(selectedUser?.username || randomMatch?.username)?.substring(0, 2).toUpperCase()}
                </div>
                {randomMatch && (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center space-x-3">
                  <h2 className="text-xl font-bold text-white truncate">
                    {selectedUser?.username || randomMatch?.username}
                    {randomMatch && (
          ...
          <div className="p-8 space-y-6">
            {randomMatch && messages.length === 0 && (
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-bold text-white mb-2">Connected with {selectedUser?.username || randomMatch?.username}!</h3>
            ...
            {isTyping && <TypingIndicator avatar={(selectedUser?.username || randomMatch?.username)?.substring(0, 2).toUpperCase()} />}
